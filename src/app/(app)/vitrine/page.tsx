...
import { useVitrineStore } from "@/stores/useVitrineStore";
import {
  useVeiculosLojaUI,
  type VeiculoLojaUI,
...
import { ESTADOS_VENDA as ESTADOS_VENDA_CONFIG } from "@/config";
...
export default function VitrinePage() {
  ...
  const handleCycleViewMode = () => {
    setViewConfig({ 
      viewMode: VIEW_MODE_ORDER[(VIEW_MODE_ORDER.indexOf(viewConfig.viewMode) + 1) % VIEW_MODE_ORDER.length] 
    });
  };
  ...
  const filtrados = useMemo(() => {
    const texto = normalizeText(filters.searchTerm);
    const parsedMin = Number(filters.precoMin);
    const parsedMax = Number(filters.precoMax);
    const min = filters.precoMin.trim() !== "" && !Number.isNaN(parsedMin) ? parsedMin : null;
    const max = filters.precoMax.trim() !== "" && !Number.isNaN(parsedMax) ? parsedMax : null;
    ...
    const aplicado = veiculosLoja.filter((item) => {
      ...
      const atendeEstado = filters.estadoFiltro ? veiculo.estado_venda === filters.estadoFiltro : true;

      if (!atendeEstado) return false;

      const atendeCaracteristica = filters.caracteristicaFiltro
        ? (veiculo.caracteristicas ?? []).some((caracteristica: { id: string }) => caracteristica?.id === filters.caracteristicaFiltro)
        : true;
  ...
  }, [veiculosLoja, filters, viewConfig.ordenacao]);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      <header 
        ...
        style={{ paddingTop: filters.searchOpen && barH > 0 ? `${barH}px` : '0px' }}
      >
  ...
  "
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() => setFilters({ filtersOpen: !filters.filtersOpen })}
                >
                  {filters.filtersOpen ? "Ocultar" : "Filtros"}
                </Button>
                ...
                {(() => {
                  const Icon = VIEW_MODE_ICON[viewConfig.viewMode];
                  return (
                    <Button
                      variant="primary"
                      size="sm"
            ...
            {filters.filtersOpen && (
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                <select
                  ...
                  onChange={(e) => setFilters({ estadoFiltro: e.target.value as EstadoVendaFiltro | "" })}
                ...
                <select
                  value={filters.caracteristicaFiltro}
                  onChange={(e) => setFilters({ caracteristicaFiltro: e.target.value })}
                  className="h-10 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 text-sm text-gray-900 dark:text-gray-100 focus:border-purple-600 focus:ring-2 focus:ring-purple-600/20 focus:outline-none transition-all"
                >
                ...
                <Input
                  ...
                  onChange={(e) => setFilters({ precoMin: e.target.value })}
                  inputSize="sm"
                />
                ...
                <Input
                  ...
                  onChange={(e) => setFilters({ precoMax: e.target.value })}
                  inputSize="sm"
                />

                <select
                  value={viewConfig.ordenacao}
                  onChange={(e) => setViewConfig({ ordenacao: e.target.value as Ordenacao })}
                  className="h-10 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 text-sm text-gray-900 dark:text-gray-100 focus:border-purple-600 focus:ring-2 focus:ring-purple-600/20 focus:outline-none transition-all"
                >
        ...
        <Button
          variant="primary"
          size="md"
          onClick={() => setFilters({ searchOpen: !filters.searchOpen })}
          className="!fixed bottom-6 right-6 z-50 !h-14 !w-14 !rounded-full !p-0 shadow-lg"
          aria-label="Alternar pesquisa"
        >
          {filters.searchOpen ? <X className="h-6 w-6" /> : <Search className="h-6 w-6" />}
        </Button>
