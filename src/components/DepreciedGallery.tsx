...
export function PhotoGallery({
  ...
  // Memoize sensor options and sensors to avoid re-registration loops in dnd-kit
  ...
  const toDisplayUrl = useCallback(
    ...
    [bucket, isPrivateBucket, supabase],
  );
...
/>
  ...
  const fotosSignature = useMemo(
    () =>
      `${isPrivateBucket ? "private" : "public"}|${fotos
        .map((f) => `${f.id}:${f.path}`)
        .join("|")}`,
    [fotos, isPrivateBucket],
  );

  const { data: urlEntries = [] } = useQuery({
    queryKey: ["foto-urls", fotosSignature, bucket, isPrivateBucket],
    queryFn: async () => {
  ...
  // Stable array of item ids for SortableContext
